...
function DiagramNavigator({ currentId, diagrams }: { currentId: number; diagrams: DiagramListItem[] }) {
    ...
    return (
        <div className="flex flex-col h-full bg-white border-l border-slate-200 w-52 shrink-0">
            {/* Header */}
            <div className="px-3 py-3 border-b border-slate-200 bg-slate-50">
                ...
                <div className="relative">
                    ...
                    <input
                        ...
                        placeholder="도면명 검색"
                        className="w-full pl-6 pr-5 py-1.5 text-xs rounded-md border border-slate-200 bg-white outline-none focus:ring-2 focus:ring-blue-400"
                    />
                    {filter && (
                ...
                {filtered.map(d => {
                    ...
                    return (
                        <Link
                            ...
                            className={`flex items-center gap-2 px-2.5 py-2 border-b border-slate-50 transition-colors ${
                                isActive
                        ...
                        >
                            {/* Thumbnail */}
                            <div className="w-10 h-10 shrink-0 rounded overflow-hidden bg-slate-100 flex items-center justify-center">
                                {d.thumbnailData
                            ...
                            <div className="flex-1 min-w-0">
                                <p className={`text-xs font-medium leading-tight line-clamp-2 ${isActive ? 'text-blue-700' : 'text-slate-700'}`}>
                                    {d.name}
                                </p>
                                <span className={`inline-block mt-0.5 px-1 py-0.5 rounded text-[10px] font-medium ${CATEGORY_COLORS[d.category] ?? 'bg-slate-100 text-slate-500'}`}>
                                    {d.category}
                                </span>
                            </div>
                        </Link>
