        ...
        <div className="max-w-4xl mx-auto p-4 md:p-6 min-h-screen">
            <header className="mb-8">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                    <div>
                        <h1 className="text-2xl sm:text-3xl font-bold mb-2 text-slate-900 dark:text-white">준공예정 관공서 및 수영장</h1>
                ...
                <div className="bg-white dark:bg-slate-800 p-4 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 flex flex-col md:flex-row md:items-center gap-3">
                    <div className="flex items-start gap-2 flex-1 w-full">
                        <Calendar className="w-5 h-5 text-slate-400 dark:text-slate-500 shrink-0 mt-2.5" />
                        <div className="flex flex-col sm:flex-row sm:items-center gap-2 flex-1">
                            <div className="flex items-center gap-2 bg-slate-100 dark:bg-slate-900 px-3 py-2 rounded-lg flex-1">
                                <span className="text-xs text-slate-400 dark:text-slate-500 shrink-0">시작</span>
                                <input
                                    type="date"
                                    value={startDate}
                                    onChange={(e) => setStartDate(e.target.value)}
                                    className="bg-transparent text-sm outline-none w-full cursor-pointer min-w-0 text-slate-900 dark:text-white dark:[color-scheme:dark]"
                                />
                            </div>
                            <span className="text-slate-400 text-sm hidden sm:block shrink-0">~</span>
                            <div className="flex items-center gap-2 bg-slate-100 dark:bg-slate-900 px-3 py-2 rounded-lg flex-1">
                                <span className="text-xs text-slate-400 dark:text-slate-500 shrink-0">종료</span>
                                <input
                                    type="date"
                                    value={endDate}
                                ...
                                <div key={bid.bidNtceNo} className="group bg-white dark:bg-slate-800 p-5 md:p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 hover:border-violet-400 dark:hover:border-violet-500 hover:shadow-md transition-all">
                                    <div className="flex flex-col md:flex-row md:items-start gap-4">
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-2 text-sm text-slate-600 dark:text-slate-300 bg-slate-50 dark:bg-slate-900/50 p-4 rounded-xl border border-transparent dark:border-slate-800">
                                                ...
                                                <div className="flex items-center justify-between md:col-span-2 pt-2 mt-2 border-t border-slate-200/60 dark:border-slate-700/60">
                                                    <span className="text-slate-400 dark:text-slate-500 flex items-center gap-1.5 shrink-0"><DollarSign className="w-3.5 h-3.5" /> <span className="hidden sm:inline">배정예산(추정가격)</span><span className="sm:hidden">추정가격</span></span>
                                                    <span className="font-bold text-base sm:text-lg text-slate-900 dark:text-white shrink-0">
                                        ...
                                        <div className="mt-4 md:mt-0 flex md:flex-col gap-2 shrink-0">
                                            <a
                                                href={bid.bidNtceUrl || 'https://www.g2b.go.kr'}
                                                target="_blank"
